...
void SignalDetectorClass::setBackupRegSD(uint32_t n) {
	enableBackupDomain();
			...
			if (MUenabled && (state == clockfound || state == syncfound) && success == false && messageLen >= minMessageLen) {
				...
				if (!m_endfound) {
				  ...
				  if (MredEnabled) {
					...
					uint16_t wr_mend;
					if (maxMuPrint < mend) {
						wr_mend = maxMuPrint;		// es werden nur maxMuPrint Daten ausgegeben
						Omend = (uint8_t)(mend / 256);
					}
					else {
						wr_mend = mend;
					}
					
					if (cMaxNumPattern < 10) {	// Pattern 0-9 -> kein hex
						uint8_t maxj = (uint8_t)((wr_mend-1) / 256);
						uint16_t maxi = 256;
						uint16_t mi = 0;
						for (uint8_t j = 0; j <= maxj; j++) {
							if (j == maxj) {
								maxi = wr_mend - (256 * maxj);	// Rest ausgeben
							}
							for (uint16_t i = 0; i < maxi; i=i+2) {
								message.getByte(mi,&n);
								mi++;
								buf[i] = (n >> 4) + '0';
								buf[i+1] = (n & 15) + '0';
							}
...
void ManchesterpatternDecoder::printBufMessageHexStr()
{
	...
	uint8_t i = 0;
	// Bytes are stored from left to right in our buffer. We reverse them for better readability
	for (idx = 0; idx < ManchesterBits.bytecount; ++idx) {
		ManchesterBits.getByte(idx,&n);
		pdec->buf[i] = hex[n >> 4];
		i++;
...
const bool ManchesterpatternDecoder::doDecode() {
	...
	while (i < pdec->messageLen-30)
	{
	  ...
	  {
		uint8_t mcStartMax = maxMCmstart - minbitlen;	// maxMCmstart = 254
		if (mcStartMax > (pdec->messageLen - minbitlen))
		{
			mcStartMax = pdec->messageLen - minbitlen;
		}
		while (i < mcStartMax)
		{
		...
		if (i >= mcStartMax)	// kein long gefunden, duerfte eigentlich nicht vorkommen
		{
			pdec->mcValid = false;
			return false;
		}
...
const bool ManchesterpatternDecoder::isManchester()
{
	...
	for (uint8_t i = 0; i < p; i++)
	{
		...
		{
#if MCDEBUGDETECT >= 1
			DBG_PRINT("i"); DBG_PRINT(i); DBG_PRINT(" x"); DBG_PRINT(x); DBG_PRINT(" ")
			...
			if ((longlow != -1) && (shortlow != -1) && (longhigh != -1) && (shorthigh != -1))
			{
				...
				while (z < pdec->messageLen)
				{
					...
					{  
						...
						DBG_PRINT("Short"); DBG_PRINT(isShort(mpz)); DBG_PRINT(";");
						if (mc_start_found == true) {
							DBG_PRINT(" mstart=");
							DBG_PRINTLN(pdec->mstart);
						}
						else {
...
#endif
						if ((z - pdec->mstart) > minbitlen && mc_start_found == true)  // Todo: Hier wird auf minbitlen geprueft. Die Differenz zwischen mstart und mend sind aber Pulse und keine bits
						{
							pdec->mend = z;
							...
							mc_start_found = false;
							if (z > (maxMCmstart - minbitlen)) {	// maxMCmstart = 254
#if MCDEBUGDETECT >= 1
								DBG_PRINTLN(" find MCmstart ismax abort");
#endif
								break;
							}
							for (uint8_t a = 0; a < 4; a++)
